...
func (g *AppGateway) GetEventByID(w http.ResponseWriter, r *http.Request) {
	...
	e := models.Event{ID: id}
	// try to select user
	if err := e.GetEvent(g.DB, g.GetUserPermissionW(w, false)); err != nil {
		switch err {
...
func (g *AppGateway) GetEventsByName(w http.ResponseWriter, r *http.Request) {
	...
	vars := mux.Vars(r)
	keyword := vars["title"]
	events, err := models.GetEventsByKeyword(g.DB, g.GetUserPermissionW(w, false), keyword, g.Config.TagPreviewLimit)
	if err != nil {
		_http.RespondWithError(w, http.StatusInternalServerError, err.Error())
		return
	}
...
func (g *AppGateway) MapTagsToEvents(w http.ResponseWriter, r *http.Request) {
	...
	// execute bulk update
	_, err := models.BulkAddTagEvent(g.DB, tem.Tags, IDs, g.GetUserPermissionW(w, false))
	if err != nil {
		_http.RespondWithError(w, http.StatusInternalServerError, "Could not bulk update documents!")
		return
	}

	media, err := models.GetEventsByIDs(g.DB, IDs, g.GetUserPermissionW(w, false))
	if err != nil {
		_http.RespondWithError(w, http.StatusInternalServerError, err.Error())
		return
	}
...
func (g *AppGateway) UpdateEventByID(w http.ResponseWriter, r *http.Request) {
	...
	e := models.Event{ID: id}
	_, err := e.UpdateEvent(g.DB, ue, g.GetUserPermissionW(w, true))
	if err != nil {
