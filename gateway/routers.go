...
func (g *AppGateway) initializeRoutes() {
	...
	g.Router.Handle("/api/v1/media/mapgroups", g.Authenticate(http.HandlerFunc(g.MapGroupsToMedia), false)).Methods("POST")
	...
	g.Router.Handle("/api/v1/mediaByHash/{ipfs_id}", g.Authenticate(http.HandlerFunc(g.GetMediaByHash), false)).Methods("GET")
	// tag
	...
	g.Router.Handle("/api/v1/usergroup/{id}/user", g.Authenticate(http.HandlerFunc(g.AddUserToUserGroupByID), false)).Methods("POST")
