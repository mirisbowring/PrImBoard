...
func (a *App) InitializeRoutes() {
	...
	// event
	a.Router.Handle("/api/v1/event", Authenticate(http.HandlerFunc(a.AddEvent))).Methods("POST")
	a.Router.Handle("/api/v1/event/{id}", Authenticate(http.HandlerFunc(a.DeleteEventByID))).Methods("DELETE")
	a.Router.Handle("/api/v1/event/{id}", Authenticate(http.HandlerFunc(a.GetEventByID))).Methods("GET")
	a.Router.Handle("/api/v1/event/{id}", Authenticate(http.HandlerFunc(a.UpdateEventByID))).Methods("PUT")
	// media
